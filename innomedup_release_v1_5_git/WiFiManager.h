 ...
 */
bool writeToMemory(String ssid, String pass,int checkInterval,int statusInterval,int gmt,int debug, int sleepOnLowBattery, int powerSaving, int weightThreshold, int binID, int displayClock){
  char buff1[30];
  char buff2[30];
  //int interval;
  ...
  EEPROM.put(300+5*sizeof(int),powerSaving);
  EEPROM.put(300+6*sizeof(int),weightThreshold);
  EEPROM.put(300+7*sizeof(int),binID);
  EEPROM.put(300+8*sizeof(int),displayClock);
  delay(100);
...
void handleSubmit(){
  ...
  if(writeToMemory(String(server.arg("ssid")),String(server.arg("password")),int(server.arg("checkInterval").toInt()),int(server.arg("statusInterval").toInt()),int(server.arg("gmtOffset_hr").toInt()),int(server.arg("debug").toInt()),int(server.arg("sleepOnLowBattery").toInt()),int(server.arg("powerSaving").toInt()),int(server.arg("weightThreshold").toInt()),int(server.arg("binID").toInt()),int(server.arg("displayClock").toInt()))){
 ...
 */
void handleRoot() {
  if (server.hasArg("ssid")&& server.hasArg("password")&& server.hasArg("checkInterval")&& server.hasArg("statusInterval")&& server.hasArg("gmtOffset_hr")&& server.hasArg("debug")&& server.hasArg("sleepOnLowBattery")&& server.hasArg("binID")&& server.hasArg("displayClock")) {
    handleSubmit();
  }
  else {
