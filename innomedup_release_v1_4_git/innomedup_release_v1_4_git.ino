...
void setup()
{
  ...
  if(!CheckWIFICreds()){
    ...
    #if scalePresent
      EEPROM.get(300+7*sizeof(int),zeroFactor);
      EEPROM.get(300+8*sizeof(int),loadCellCalibrationInt);
      loadCellCalibration=float(loadCellCalibration/10);
  ...
  //rtc_gpio_deinit(to_gpio(WiFiButton));
...
//    Serial.println("INPUT PULLUP");
  ...
  if(newCredentials){
    ...
    #if(oledPresent)
    {
      ...
      displayDebugMessage("Your MAC is:",0,0,false,0,false);
      displayDebugMessage(macString,0,10,true,15000,true);
    }
    #endif
...
void loop(){
  if(!digitalRead(WiFiButton)){ //if WiFi button is pressed for 3 seconds or more enter WiFi input credentials mode
    ...
    if(!digitalRead(WiFiButton)){ //if button is pressed for 3 seconds 
      ...
      wipeEEPROM();
      #if scalePresent
        loadCellCalibration=int(loadCellCalibration*10);
        EEPROM.put(300+7*sizeof(int),zeroFactor);
        EEPROM.put(300+8*sizeof(int),loadCellCalibrationInt);
        EEPROM.commit();
      #endif
  ...
  if(hardReset){ //When turned on check if Bin has been registered before
    ...
    delay(10);
    #if scalePresent
      weight=constrain(readScaleData2(true),0,200000);
    #else 
      weight=0;
    #endif
    ...
    #if(scalePresent)
    {
      ...
      if(weight>=weightThreshold) {
        ...
        isFull=false;
        overWeight=false;
        //Only if us is not presented we will upload that the bin is empty just from the weight.
        if(!usPresent){
          if (previouslyFull!=isFull){
          previouslyFull=isFull;
          binIsFull(debug,isFull);
  ...
  sleepCounter++;
  // #if scalePresent
  //   esp_sleep_enable_ext1_wakeup(to_gpio(scaleButton), 0); //wake up if WiFi button is pressed
  // #endif
  esp_sleep_enable_ext0_wakeup(to_gpio(WiFiButton), 0); //wake up if WiFi button is pressed
  uint64_t unsignedCheckInterval=(uint64_t)checkInterval*60000000;
  esp_sleep_enable_timer_wakeup(unsignedCheckInterval); //set timer to wake up the esp after the defined time
...
void binIsFull(bool debug,bool emptyOrFull){
  //connect to WiFi
  #if oledPresent  
    displayDebugMessage(emptyOrFull?"Your Bin is Full":"Your Bin is Empty",0,0,false,10000,true);
  #endif
  connectWiFi(debug,ssidString.c_str(),passwordString.c_str());
  char timeNow[40];
  getTime(timeNow,&timestruct);
  ...
  #if scalePresent
    #if batteryMeasurement
      String fireVariables[4] = {"isFull","batteryLevel",emptyOrFull?"timeFilled":"timeEmptied","weight"};
      String fireValues[4] = {emptyOrFull?"yes":"no",String(batteryPercentage),String(timeNow)+" "+gmtOffset_str,String(weight)};
      setFirebaseFirestoreValue(fireVariables,fireValues,4);
    #else
      String fireVariables[3] = {"isFull",emptyOrFull?"timeFilled":"timeEmptied","weight"};
      String fireValues[3] = {emptyOrFull?"yes":"no",String(timeNow)+" "+gmtOffset_str,String(weight)};
...
#if oledPresent
void displayDebugMessage(String message, int posx, int posy, bool addMessage, int offTime, bool turnOffAfterwards){
...
void scaleCalibrationStart(){
  displayDebugMessage("Scale Calibration",0,0,false,0,false);
  displayDebugMessage("Remove weight, press BTN",0,10,true,0,false);
  ...
  displayDebugMessage("Place a known weight",0,0,false,0,false);
  displayDebugMessage("Press buttons until accurate.",0,10,true,0,false);
  displayDebugMessage("When done press both buttons",0,20,true,5000,false);
  while((digitalRead(scaleButton))&&(digitalRead(scaleButton))); //wait to press button
  delay(100);
  while(true){
    int units = scale.get_units();
    displayDebugMessage(String(units)+"gr",0,0,false,0,false);
    displayDebugMessage(String(loadCellCalibration),0,10,true,0,false);
    ...
    if(!digitalRead(scaleButton)) loadCellCalibration++;
    if(!digitalRead(WiFiButton)) loadCellCalibration--;
    delay(50);
  }
  EEPROM.put(300+7*sizeof(int),zeroFactor);
  loadCellCalibrationInt=int(loadCellCalibration*10);
  EEPROM.put(300+8*sizeof(int),loadCellCalibration);
  EEPROM.commit();
